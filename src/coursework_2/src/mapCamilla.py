...
def expand_map(img, robot_width):
    ...
    ############################################################### TASK A
    # SQUARE MASK
    w = DENIRO_width*scale                   # width of DENIRO
    robot_square_mask = np.ones((w, w))      # square array of ones the size of the robot
    
    #print(robot_square_mask)
    #expanded_img = binary_dilation(img, robot_square_mask)
    
    # CIRCULAR MASK
    mask_width = 16
    
    robot_circular_mask = np.ones((mask_width, mask_width))

    mask_radius = mask_width/2

    for y, v_y in enumerate(robot_circular_mask):
        for x, v_x in enumerate(v_y):
            if (np.square(x - mask_radius + 0.5) + np.square(y - mask_radius + 0.5)) < np.square(mask_radius):

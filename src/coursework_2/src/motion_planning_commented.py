...
# NB: 
# Walls in world coordinates go from -10.1 -> 10.1, but with 16 scale, map is -320.32 -> 320.32
# The pixel_map array is 340x340, so there are some 1s either side of these walls
# Centre of the pixel coordinates is left bottom corner of the C-map
# Centre of the world coordinates is at the centre of the C-map
...
def set_vref_publisher():                                       # publisher for DENIRO odometry
    ...
    # NB: "Twist" breaks down velocity into angular and linear components
    ...
    # NB: Odometry message content: http://docs.ros.org/en/melodic/api/nav_msgs/html/msg/Odometry.html
    # when the "odom" message is received, the deniro_odom_callback() is invoked with the message as the first argument
    return pub

def cmd_vel_2_twist(v_forward, omega):                          # decoding the Twist() message from ROS.
    twist_msg = Twist()                                         # See documentation: http://docs.ros.org/en/lunar/api/geometry_msgs/html/msg/Twist.html 
...
class MotionPlanner():
    ...
    def run_planner(self, planning_algorithm):                  # run the chosen planning algorithm
        ...
        while not rospy.is_shutdown():                          # testing for shutdown
            ...
            if complete:                                        # if goaal is reached
                print("Completed motion")
                break                                           # stop running the planenr
            rate.sleep()                                        # pause for a little
    
    def setup_waypoints(self):
        ############################################################### REPORT SECTION 3.1 and 3.2
        waypoints = np.array([[2.00, -2.30],                    # an array of waypoints for the robot to navigate via to reach the goal
                              [3.87, 3.03],                     # these values are for the pink path
                              [7.75, 4.66]])
    ...
    def waypoint_navigation(self):
        ...
        current_waypoint = self.waypoints[self.waypoint_index, :]   # get the current waypoint
        waypoint_vector = current_waypoint - deniro_position        # calculate the vector from DE NIRO to waypoint
        distance_to_waypoint = np.linalg.norm(waypoint_vector)      # calculate the distance from DE NIRO to waypoint
        waypoint_direction = waypoint_vector / distance_to_waypoint # calculate the unit direction vector from DE NIRO to waypoint
        
        vref = waypoint_direction * 0.5                         # calculate a reference velocity based on the direction of the waypoint
        
        # If waypoint is reached, start moving to the next waypoint
        if distance_to_waypoint < 0.05:
            self.waypoint_index += 1                            # increase waypoint index
            
        # If last waypoint is reached, stop
        if self.waypoint_index >= self.waypoints.shape[0]:      # Note: this needed to be changed to bigger than
    ...
    def potential_field(self):
        ...
        goal_vector = goal - deniro_position                    # vector to goal position from DE NIRO
        distance_to_goal = np.linalg.norm(goal_vector)          # distance to goal position from DE NIRO
        pos_force_direction = goal_vector / distance_to_goal    # unit vector in direction of goal from DE NIRO
        
        # Potential function for attractive (positive) force (to goal)
        #pos_force_magnitude = 1/distance_to_goal               #------ uncomment and comment the line below for the potential field in report section 4.1
        pos_force_magnitude = 1/np.square(distance_to_goal)     # the improved potential field in report section 4.2
        ...
        #force_magnitude = -1/distance_to_obstacle              #------ uncomment and comment the line below for the potential field in report section 4.1
        force_magnitude = -1/np.square(distance_to_obstacle)    # the improved potential field in report section 4.2
        ...
        # Make sure to comment it out again when motion planner is run, otherwise it won't work
...
#        plt.show()
    ...
    def generate_random_points(self, N_points): # N_points are the number of generated points in free space we want (==100 in this case)
        ...
        # Keep generating points until N_points have been generated that are in free space (ie not in an obstacle)
        while N_accepted < N_points:
            ...
            for i in range(len(pixel_points)):                  # loop through all the randomly generated pixel map points
                if self.pixel_map[int(pixel_points[i][1]), int(pixel_points[i][0])]: # find the point in the pixel map, if the point is in an obstacle:
                    rejected[i] = 1                             # point will be added to the rejected array.

            new_accepted_points = pixel_points[np.argwhere(rejected == 0)].reshape((-1, 2)) # finding all accepted (0) and rejected (1) points in pixel_points and reshaping to be an array of points
        ...
        for i in range(points.shape[0]):                        # loop through each node
            ...
            if points_in_range.shape[0] > 0:                    # if there are any nodes in an acceptable range
                ...
                for j in range(points_in_range.shape[0]):       # loop through the nodes an acceptable distance of the current node
                    ...
                    if collision:                               # if there is a collision, plot the edge in red
                        plt.plot([pxA[0, 0], pxB[0, 0]], [pxA[0, 1], pxB[0, 1]], c='r')
                        pass
                    else:                                       # if there is no collision, add the node in range to the array of nodes that have no collisions
                        ...
                        plt.plot([pxA[0, 0], pxB[0, 0]], [pxA[0, 1], pxB[0, 1]], c='b') # plot the edge in blue
    ...
    def check_collisions(self, pointA, pointB):
        ...
        resolution = 0.0005                                     # tuned resolution for collision checking
    ...
    def dijkstra(self, graph, edges):
        ...
        initial_cost = 1000000                                  # initial_cost set to a very high number 
